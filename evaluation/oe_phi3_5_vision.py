...
# python evaluation/oe_phi3_5_vision.py
# nohup python evaluation/oe_phi3_5_vision.py >> results/output_oe_phi3_5_vision.log 2>&1 &

# CUDA_VISIBLE_DEVICES=1 nohup python evaluation/oe_phi3_5_vision.py >> results/output_oe_phi3_5_vision.log 2>&1 &
...
DTYPE = "auto"  # Use 'auto' as recommended in model documentation
...
def generate_answer(model, processor, images, prompt, retries=3):
    ...
    for attempt in range(1, retries + 1):
        try:
            inputs = processor(full_prompt, images, return_tensors="pt").to(device=DEVICE)
            generation_args = {
                "max_new_tokens": 100,
                "temperature": 0.2,
...
def main():
    ...
    # Load processors for single and multi-image (as recommended: num_crops=16 for single, 4 for multi)
    print(f"[INFO] Loading processors for single and multi-image modes")
    processor_single = AutoProcessor.from_pretrained(
        MODEL_NAME, 
        trust_remote_code=True, 
        num_crops=16  # For single-frame as per model recommendations
    )
    processor_multi = AutoProcessor.from_pretrained(
        MODEL_NAME, 
        trust_remote_code=True, 
        num_crops=4  # For multi-frame as per model recommendations
    )
    ...
    for image_entry in dataset:
        ...
        for question_entry in image_entry['questions']:
            ...
            # Select appropriate processor based on number of images
            if isinstance(images, list) and len(images) > 1:
                processor = processor_multi
            else:
                processor = processor_single
                if not isinstance(images, list):
                    images = [images]  # Ensure list for consistency
