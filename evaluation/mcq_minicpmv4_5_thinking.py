# Note: done in deterministic setting: sampling=False, temperature=0 because MCQ
# enable_thinking=True

# Revisions made for thinking model:
# - Updated INSTRUCTION prompt to include concise thinking and 'Final Answer: X' format for better response structure.
# - Enhanced parse_model_response with priority on 'Final Answer: X' and fallback to last matching A-E in verbose outputs.
# - Added max_new_tokens=3000 to model.chat() call to prevent truncation in thinking-enabled responses.
...
# python evaluation/mcq_minicpmv4_5_thinking.py
# CUDA_VISIBLE_DEVICES=1 nohup python evaluation/mcq_minicpmv4_5_thinking.py >> results/output_mcq_minicpmv4_5_thinking.log 2>&1 &
...
# Model configuration
SEED = 0
MODEL_NAME = "openbmb/MiniCPM-V-4_5"  # Updated to MiniCPM-V-4.5
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
DTYPE = torch.bfloat16 if torch.cuda.is_available() else torch.float32

# Paths
INPUT_JSON_PATH = "RobustMAD_MCQ.json" #"RobustMAD_MCQ_low"    # Input dataset JSON
model_name_short = MODEL_NAME.split("/")[-1].replace("-", "_")

# Suffix determination
if "low" in INPUT_JSON_PATH.lower(): # "Robust_Anomaly_VQA_low_AA.json" for low-quality images
    DATASET_SUFFIX = "_low"        
else:
    DATASET_SUFFIX = ""            # "Robust_Anomaly_VQA_AA.json" for high-quality images
...
# Instruction prompt to ensure the model responds with reasoning and the letter
# Updated to trigger concise reasoning and end with 'Final Answer: X'
INSTRUCTION = (
    "You are an expert industrial product quality inspector. Given the image(s) and the question below, think step by step **silently** about the content and reasoning required. "
    "Then, select the single correct option."
    "For the actual response, only give a short reasoning (2-3 sentences max) and end your response with 'Final Answer: X' where X is the corresponding letter (A, B, C, D, or E). "
    "No other output after the final answer."
)
...
def parse_model_response(response):
    """Fool-proof extraction of answer choice A-E from model response, handling reasoning output."""
    if not response:
    ...
    final_match = re.search(r'FINAL ANSWER:\s*([A-E])', text)
    if final_match:
        return final_match.group(1)
    
    # Comprehensive patterns as fallback, find the last match
    patterns = [
        r'\b([A-E])\b',
        r'\b([A-E])(?=[\.\,\)\]\}\>\:\s]|$)',
    ...
    # Last resort: last [A-E] in text
    fallback_matches = [m for m in re.finditer(r'[A-E]', text)]
    if fallback_matches:
        return fallback_matches[-1].group(0)
    ...
    for attempt in range(1, retries + 1):
        try:
            answer = model.chat(
                msgs=msgs,
                tokenizer=tokenizer,
                sampling=False,  # Deterministic output
                temperature=0,
                enable_thinking=True,  # Enable deep thinking mode for reasoning
                max_new_tokens=3000  # Increased to prevent truncation of reasoning and final answer
            )
...
def compute_and_log_accuracy(output_data):
    ...
    df['object_specie_pair'] = df['image_path'].apply(lambda x: "-".join([x.split("/")[1], x.split("/")[3]])) # e.g., "bottle-good" based on path
    ...
    # Step 2: Compute Overall average per robustness category + Average per robustness category × difficulty ("general" / "domain")
