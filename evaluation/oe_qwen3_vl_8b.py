...
# python evaluation/oe_qwen3_vl_8b.py
# nohup python evaluation/oe_qwen3_vl_8b.py >> results/output_oe_qwen3_vl_8b.log 2>&1 &

# CUDA_VISIBLE_DEVICES=1 nohup python evaluation/oe_qwen3_vl_8b.py >> results/output_oe_qwen3_vl_8b.log 2>&1 &

# conda activate qwen3vl_venv  # Adjust as needed based on your environment
...
# Model configuration
SEED = 0
MODEL_NAME = "Qwen/Qwen3-VL-8B-Instruct"  # As per the model card: https://huggingface.co/Qwen/Qwen3-VL-8B-Instruct
...
def save_and_print_prompt_preview(content_list, full_prompt):
    ...
    for item in content_list:
        if item['type'] == 'image':
            lines.append(f"  Image {image_count}: PIL.Image object [not shown]")
            image_count += 1
...
def generate_answer(model, processor, images, prompt, retries=3):
    ...
    # Construct the full prompt text (instruction + question)
    full_prompt = (
        f"{INSTRUCTION}\n\n"
        f"Question: {prompt}"
    )

    # Prepare content list: images + text
    content = []
    ...
    save_and_print_prompt_preview(content, full_prompt)

    start_time = time.time()

    for attempt in range(1, retries + 1):
        try:
            # Process with apply_chat_template (handles tokenization and multimodal inputs directly)
...
def main():
    ...
    for image_entry in dataset:
        ...
        for question_entry in image_entry['questions']:
            ...
            # Generate response
            mslm_answer = generate_answer(model, processor, images, question)
