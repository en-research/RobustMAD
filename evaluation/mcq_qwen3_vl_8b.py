# evaluation/mcq_qwen3_vl_8b.py
# Deterministic generation: do_sample=False, temperature=0
# max_new_tokens=10 for short response (just the letter)
# Tips from https://github.com/QwenLM/Qwen3-VL and HF model card: 
# - For Qwen3-VL series (including 8B), use the updated processor.apply_chat_template which handles multimodal inputs directly.
# - No need for separate process_vision_info; processor manages images/videos internally.
...
# python evaluation/mcq_qwen3_vl_8b.py
...
# Model configuration
SEED = 0
MODEL_NAME = "Qwen/Qwen3-VL-8B-Instruct"  # Updated to 8B model
...
def generate_answer(model, processor, images, prompt, retries=3):
    ...
    for attempt in range(1, retries + 1):
        try:
            # Process with apply_chat_template (handles tokenization and multimodal inputs)
...
def main():
    ...
    model = Qwen3VLForConditionalGeneration.from_pretrained(
        MODEL_NAME,
        torch_dtype=DTYPE,
        device_map="auto",  # Use "auto" for better handling of larger 8B model
